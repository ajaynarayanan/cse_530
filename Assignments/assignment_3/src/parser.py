from constants import LOGS_ROOT_DIR, X86_MINOR_PARAM_PATTERNS, X86_TIMING_PARAM_PATTERNS
from constants import X86MinorCPU
...
def collect_experiments():
    ...
    results = {}
    for file in files:
        exp_name = file.split("/")[-1][3:-4]
        print(f"Parsing data for {exp_name}")
        experiment = Experiment(exp_name)
        if experiment.cpu_type == X86MinorCPU:
            results[experiment] = collect_all_params(file, X86_MINOR_PARAM_PATTERNS)
        else:
            results[experiment] = collect_all_params(file, X86_TIMING_PARAM_PATTERNS)
    return results
